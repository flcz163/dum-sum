 ...
 * sys/sysmacros.h
 *
 * Constructs to create and pick apart devno_t.  The double-underscore
...
__static_inline int _major(devno_t __d)
{
	return __major(__d);
}
...
__static_inline int _minor(devno_t __d)
{
	return __minor(__d);
}
...
#define __makedev(__ma, __mi) \
	((devno_t)((((__ma) & 0xfffU) << 8)| \
		 ((__mi) & 0xffU)|(((__mi) & 0xfff00U) << 12)))
__static_inline devno_t _makedev(int __ma, int __mi)
{
