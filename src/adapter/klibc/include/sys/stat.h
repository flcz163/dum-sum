...
__extern int mknod(const char *, mode_t, devno_t);
__extern int mknodat(int, const char *, const char *, mode_t, devno_t);
__extern int mkfifo(const char *, mode_t);

__extern_inline int mkfifo(const char *__p, mode_t __m)
{
	return mknod(__p, (__m & ~S_IFMT) | S_IFIFO, (devno_t) 0);
}
