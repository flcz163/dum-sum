...
static inline void atomic_set_bit(unsigned long *bitmap, unsigned int bit)
{
...
int __attribute__((weak)) vsscanf(const char *buffer, const char *format, va_list ap)
{
	const char *p = format;
	char ch;
	unsigned char uc;
	...
	while ((ch = *p++) && !bail) {
		...
		case st_match_init:	/* Initial state for %[ match */
			if (ch == '^' && !(flags & FL_INV)) {
				...
				atomic_set_bit(matchmap, (unsigned char)ch);
				state = st_match;
			}
			break;
		...
		case st_match_range:	/* %[ match after - */
			if (ch == ']') {
				/* - was last character */
				atomic_set_bit(matchmap, (unsigned char)'-');
				goto match_run;
			} else {
				int i;
				for (i = range_start; i < (unsigned char)ch;
				     i++)
					atomic_set_bit(matchmap, i);
				state = st_match;
			}
			break;
