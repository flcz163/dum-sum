#ifndef _DIM_SUM_WORKQUEUE_H
#define _DIM_SUM_WORKQUEUE_H
...
#define INIT_WORK(_work, _func, _data)				\
	do {							\
		list_init(&(_work)->entry);		\
		(_work)->pending = 0;				\
		PREPARE_WORK((_work), (_func), (_data));	\
		timer_init(&(_work)->timer);			\
	} while (0)
...
extern void init_sleep_works(void);
...
static inline int cancel_delayed_work(struct work_struct *work)
{
	...
	ret = synchronize_timer_del(&work->timer);
	if (ret)
		atomic_clear_bit(0, &work->pending);
	return ret;
}
#endif /* _DIM_SUM_WORKQUEUE_H */