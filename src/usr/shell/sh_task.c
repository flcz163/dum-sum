 ...
 * 是否是有效任务号
 */
int is_valid_task(unsigned long task_id)
{
 ...
 * 获取初始任务号
 */
static unsigned long get_init_task_id(void)
{
 ...
 * 获取下一个任务号
 */
static unsigned long get_next_task_id(unsigned long prev_task_id)
{
 ...
 * 获取任务名称
 */
char *get_task_name(unsigned long task_id)
{
 ...
 * 获取任务入口地址
 */
static unsigned long get_task_entry(unsigned long task_id)
{
 ...
 * 获取任务优先级
 */
static int get_task_priority(unsigned long task_id)
{
 ...
 * 获取任务状态
 */
static int get_task_state(unsigned long task_id)
{
 ...
 * 打印任务栈桢
 */
static void dump_task(unsigned long task_id)
{
 ...
 * 任务是否暂停运行
 */
int task_is_suspend(unsigned long task_id)
{
	int task_state = UNKNOWN_STATE;
 ...
 * 输出任务信息标题
 */
static void show_task_info(unsigned long task_id)
{
	char sym_name[128] = {0x0, };

	SH_PRINTF("0x%-16lx %-16s 0x%-16lx %-25.25s %-4d %-8s\n",
		task_id,
...
static int sh_show_task_cmd(int argc, char *argv[])
{
	...
	if (argc == 1)
	{
		unsigned long prev_task_id = 0, next_task_id = 0;
...
static int sh_suspend_task_cmd(int argc, char *argv[])
{
			...
			|| task_state == TASK_SUSPEND)
	{
		SH_PRINTF("Task <0x%x> has been suspended.\n", task_id);
			...
			|| task_state == TASK_INTERRUPTIBLE)
	{
		ret = sh_ker_suspend_task(task_id);
		if (ret < 0)
		{
...
{
	create_process(test_task, NULL, "test_task", 15);
