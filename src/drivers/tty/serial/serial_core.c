...
static struct semaphore port_sem = 
			SEMAPHORE_INITIALIZER(port_sem, 1);
...
static struct uart_state *uart_get(struct uart_driver *drv, int line)
{
	...
	if (!state->info) {
		state->info = kmalloc(sizeof(struct uart_info), PAF_KERNEL);
		if (state->info) {
			memset(state->info, 0, sizeof(struct uart_info));
			init_waitqueue(&state->info->open_wait);
			init_waitqueue(&state->info->delta_msr_wait);
...
static int uart_startup(struct uart_state *state, int init_hw)
{
	 ...
	 */
	if (info->tty)
		atomic_set_bit(TTY_IO_ERROR, &info->tty->flags);
	 ...
	 */
	if (!info->xmit.buf) {
		page = alloc_zeroed_page_memory(PAF_KERNEL);
		if (!page)
			return -ENOMEM;
	...
	if (retval == 0) {
		...
		atomic_clear_bit(TTY_IO_ERROR, &info->tty->flags);
	}
...
uart_block_til_ready(struct file *filp, struct uart_state *state)
{
	...
	add_to_wait_queue(&info->open_wait, &wait);
	while (1) {
	...
	del_from_wait_queue(&info->open_wait, &wait);
...
static void uart_shutdown(struct uart_state *state)
{
	 ...
	 */
	if (info->xmit.buf) {
		free_page_memory((unsigned long)info->xmit.buf);
		info->xmit.buf = NULL;
	}
...
uart_wait_modem_status(struct uart_state *state, unsigned long arg)
{
	...
	smp_unlock_irq(&port->lock);

	add_to_wait_queue(&state->info->delta_msr_wait, &wait);
	for (;;) {
		smp_lock_irq(&port->lock);
	...
	del_from_wait_queue(&state->info->delta_msr_wait, &wait);
...
/*
 * Called via sys_ioctl under the BKL.  We can use smp_lock_irq() here.
 */
static int
...
#define RELEVANT_IFLAG(iflag)	((iflag) & (IGNBRK|BRKINT|IGNPAR|PARMRK|INPCK))
	...
	if ((old_termios->c_cflag & CRTSCTS) && !(cflag & CRTSCTS)) {
		smp_lock_irqsave(&state->port->lock, flags);
		tty->hw_stopped = 0;
		__uart_start(tty);
		smp_unlock_irqrestore(&state->port->lock, flags);
	}
...
static void uart_wait_until_sent(struct tty_struct *tty, int timeout)
{
	...
	while (!port->ops->tx_empty(port)) {
		...
		if (signal_pending(current))
			break;
		if (time_after((unsigned long)jiffies, expire))
			break;
	}
...
int uart_register_driver(struct uart_driver *drv)
{
	...
	/*
	 * Maybe we should be using a Beehive cache for this, especially if
	 * we have a large number of ports to handle.
	 */
	drv->state = kmalloc(sizeof(struct uart_state) * drv->nr, PAF_KERNEL);
	retval = -ENOMEM;
	if (!drv->state)
		goto out;
	...
	normal->sysobj_name	= drv->sysobj_name;
...
int uart_add_one_port(struct uart_driver *drv, struct uart_port *port)
{
	 ...
	 */
	if (port->type != PORT_UNKNOWN &&
	    port->cons && !(port->cons->flags & CONSOLE_ENABLED))
...
{
	struct tty_struct *tport = port->info->tty;
 ...
 *	flags), and space the %B0 rate to 9600 baud.
 *
