...
struct smp_lock lwip_spin_lock;
...
err_t sys_mbox_new(sys_mbox_t *mbox, int size)
{
	...
	msg_queue_init(&mbox->sys_mbox, LWIP_MAX_MSGS, size, 0);// = ker_msgQCreate(LWIP_MAX_MSGS, size, 0);
	mbox->valid = 1;
	return ERR_OK;
}
...
{
	msg_queue_destroy(&mbox->sys_mbox);
}
...
void sys_mbox_post(sys_mbox_t *mbox, void *msg)
{
	...
	if (msg_queue_send(&mbox->sys_mbox, (char*)&msg, sizeof(void*), WAIT_FOREVER, 0))
...
{
	return msg_queue_send(&mbox->sys_mbox, (char*)&msg, sizeof(void*), 0, 0) ? ERR_MEM : ERR_OK;
}
...
u32_t sys_arch_mbox_fetch(sys_mbox_t *mbox, void **msg, u32_t timeout)
{
	...
	start = uptime();
	if (msg_queue_receive(&mbox->sys_mbox, (char*)msg, sizeof(void*), wait) <= 0) {
...
{
	return msg_queue_receive(&mbox->sys_mbox, (char*)msg, sizeof(void*), 0) ? SYS_MBOX_EMPTY : 0;
}
...
err_t sys_mutex_new(sys_mutex_t *mutex)
	{
	mutex_init(&mutex->sys_mutex);// = ker_semMCreate(0);
#if 0
	if (mutex->sys_mutex) {
...
void sys_mutex_lock(sys_mutex_t *mutex)
{
	...
	mutex_lock(&mutex->sys_mutex);
}

void sys_mutex_unlock(sys_mutex_t *mutex)
{
	mutex_unlock(&mutex->sys_mutex);
}
...
{
	sema_init(&sem->sys_sem, count);// = ker_semCCreate(0, (int)count);
#if 0
	if(sem->sys_sem){
...
u32_t sys_arch_sem_wait(sys_sem_t *sem, u32_t timeout)
{
	...
	ret = down_timeout(&sem->sys_sem, timeout);
	if (ret == -1) {
		return SYS_ARCH_TIMEOUT;
	}
	if (ret == 0)
...
sys_thread_t sys_thread_new(const char *name, lwip_thread_fn thread, void *arg, int stacksize, int prio)
{
	...
	if ((tid = create_process(entry, arg, (char *)name, prio)) == 0) {
...
int init_lwip(void)
{
	//create_process(__dim_sum_lwip_init, NULL, "ph_lwip_init", 1, NULL, 0);
	__dim_sum_lwip_init(NULL);
