...
extern struct smp_lock lwip_spin_lock;
#define SYS_ARCH_DECL_PROTECT(x) unsigned long x
#define SYS_ARCH_PROTECT(x)      smp_lock_irqsave(&lwip_spin_lock, x)
#define SYS_ARCH_UNPROTECT(x)    smp_unlock_irqrestore(&lwip_spin_lock, x)


/*** 将内存配置为MEM_LIBC_MALLOC=1 ，并将mem_malloc和mem_free重定义为dim-sum的内存函数 ***/
extern void __mem_free(void *p);
#define mem_malloc(x) kmalloc_app(x)
